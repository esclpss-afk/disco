...
BAN_LOG_PORT = int(os.getenv("PORT", os.getenv("BAN_LOG_PORT", "8081")))
...
async def handle_health(request: web.Request) -> web.Response:
    return web.json_response({"ok": True, "service": "discord_hide_bot", "ban_log": True})
...
async def main() -> None:
    ...
    if not BOT_TOKEN:
        print("Missing DISCORD_BOT_TOKEN; HTTP server running without Discord connection.")
    ...
    try:
        ...
        print(f"Discord bot start failed: {error}")
        print("Keeping HTTP server alive for health/debug endpoint.")
        await asyncio.Event().wait()
    finally:
